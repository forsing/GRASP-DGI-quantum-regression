...
# Varijanta: DGI (Deep Graph Infomax) – GCN patch + readout summary, poravnanje bez učenja
 
"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data
DGI = encoder (1 GCN sloj) + readout summary; ovde H = A_norm @ H0, emb poravnat sa summary
"""
...
DGI_SCALE = 0.5 # skalirajuca konstanta
...
def compute_dgi_embeddings(M, k=EMBED_DIM):
    ...
    s = H.mean(axis=0)
    align = H @ s
    align = (align - align.min()) / (align.max() - align.min() + 1e-12)
    emb = H * (1.0 + DGI_SCALE * align[:, np.newaxis])
...
def main():
    ...
    emb = compute_dgi_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (DGI + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    print("Score:", pred_scores[best_combo[0]])
    print()
    """
    Predikcija (DGI + kvantna regresija, deterministički, strukturno):
    (2, 8, 10, 22, 23, 33, 34)

    Score: 0.5053022956855858
    """
...
Jedan GCN sloj: H = A_norm @ H0 (patch reprezentacije).

Readout: s = mean(H) po čvorovima (graph summary).

Poravnanje: align = H @ s, normalizovano u [0,1]; 
emb = H * (1 + DGI_SCALE * align) 
(čvorovi usklađeni sa summary jače se pojačavaju).

Zatim normalizacija emb u [0, π].
"""
